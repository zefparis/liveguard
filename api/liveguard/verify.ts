/**
 * LiveGuard — Hardened Vercel proxy handler
 *
 * POST /api/liveguard/verify
 *
 * Security:
...
import { sanitizeResponse } from '../_lib/liveguardSanitize.js';
...
function getTenantId(): string {
  return process.env.LIVEGUARD_TENANT_ID || 'liveguard-demo';
}

function getRateLimitPerMin(): number {
  const raw = process.env.LIVEGUARD_PROXY_RATE_LIMIT_PER_MIN;
  if (!raw) return 0;
...
export default async function liveguardVerifyHandler(
  ...
  safeLog('info', {
    event: 'liveguard_behavior_signal',
  ...
  try {
    ...
    try {
      ...
      res.status(502).json(safeError('LiveGuard verification unavailable'));
      return;
    }
    ...
    (safeResponse as Record<string, unknown>).source = 'liveguard_mobile';
