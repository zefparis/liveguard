/**
 * LiveGuard Cognitive Battery — Global scoring & summary
 *
 * computeCognitiveSummary() aggregates all 5 module results into
...
// LiveGuard: 5-module denominator (vocal_ran removed). The original
// demoguard used 6 modules; LiveGuard uses 5 (reflex, stroop, digit_span,
// n_back, trail_tap). The denominator change from 6→5 is the only scoring
// adjustment — all other scoring logic is identical to demoguard.
export function computeCognitiveSummary(signals: CognitiveSignals): CognitiveSummary {
  const modules = [signals.reflex, signals.stroop, signals.digit_span, signals.n_back, signals.trail_tap];
  ...
  if (completedModules > 0) {
    ...
    const completionRatio = completedModules / 5;
